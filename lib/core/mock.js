...
function request(
  ...
  const fileName = !options.exact ? urlObj.pathname : '';
  const filePath = path.join(cwd, options.mockPath, fileName);
  ...
  return delay(options.delay).then(() => {
    const extname = path.extname(fileName);
    if (extname === '.json') {
      return resJSON(req, res, filePath);
    }
    return resHtml(req, res, filePath);
  });
}
